...
import geom_transform_toolkit as geom_transform


def weightedOrthoError(W, p2d, p3d, theta, optimize_s):

    R = geom_transform.angle_axis_to_rotation_matrix(theta[..., :3])
    if optimize_s:
        s = theta[..., 3:4]#.relu()+0.1#torch.nn.functional.relu(theta[..., 3:4], inplace=False)+0.001
        t = theta[..., 4:]
    else:
        s = 1.
        t = theta[..., 3:]
    ...
    p3d_rot = torch.einsum('brs,bms->bmr', (R, p3d))
    #transform_points_by_theta(p, theta)
    p3d_rot = s.unsqueeze(1) * p3d_rot
    proj_xy = p3d_rot[...,:2] + t.unsqueeze(1)
    b = p2d.size(0)
    m = p2d.size(1)
    n = p3d.size(1)
    W = W.reshape(b, m, n)
    ...
    p2d_xy = p2d[..., :2]
    #dist = ((p2d_xy - proj_xy) ** 2).sum(dim=-1)
    dist = (p2d_xy ** 2).sum(dim=-1, keepdim=True) + (proj_xy ** 2).sum(dim=-1, keepdim=True).permute(0,2,1) - 2 * torch.einsum('bmd,bnd->bmn', (p2d_xy, proj_xy))
    #print(dist.shape)
    error = torch.einsum('bmn,bmn->b', (W, dist))
    #error = (W * dist).sum(dim=-1)
    #print(error.shape)
    return error
...
def weightedAngularReprojectionError(W, p2d, p3d, theta):
    ...
    n = p3d.size(1)
    W = W.reshape(b, m, n)
    p3dt = geom_transform.transform_and_normalise_points_by_theta(p3d, theta)
...
class NonlinearWeightedOrthBlindPnPFn(torch.autograd.Function):
    """
    A class to optimise the weighted angular reprojection error given
    a set of 3D point p3d, a set of bearing vectors p2d, a weight matrix W
    """
    ...
    def forward(ctx, W, p2d, p3d, theta0=None):
        ...
        with torch.enable_grad():
            ...
            def closure():
                if torch.is_grad_enabled():
                    opt.zero_grad()
                error = weightedOrthoError(W, p2d, p3d, theta, optimize_s=True).mean()  # average over batch
                if error.requires_grad:
                    error.backward()
                return error
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:  # W only
            with torch.enable_grad():
                W = W.detach().requires_grad_()
                theta = theta.detach().requires_grad_()
                fn_at_theta = weightedOrthoError(W, p2d, p3d, theta, optimize_s=True)  # b
            ...
            grad_input_diag = []
            for i in range(b):
                grad_input_diag.append(grad_input[i].diag())

            grad_input = torch.stack(grad_input_diag)

        return grad_input, None, None, None


class NonlinearWeightedOrthNosBlindPnPFn(torch.autograd.Function):
    """
    A class to optimise the weighted angular reprojection error given
    a set of 3D point p3d, a set of bearing vectors p2d, a weight matrix W
    """
    ...
    def forward(ctx, W, p2d, p3d, theta0=None):
        ...
        Arguments:
            ...
            p3d: (b, n, 3) Torch tensor,
                batch of 3D point-sets
            theta0: (b, 5) Torch tensor,
                batch of initial transformation parameters
                assumptions:
                    theta[:, 0:3]: angle-axis rotation vector
                    theta[:, 3:5]: translation vector
        Return Values:
            theta: (b, 5) Torch tensor,
                batch of optimal transformation parameters
        """
        W = W.detach()
        p2d = p2d.detach()
        ...
        with torch.enable_grad():
            ...
            def closure():
                if torch.is_grad_enabled():
                    opt.zero_grad()
                error = weightedOrthoError(W, p2d, p3d, theta, optimize_s=False).mean()  # average over batch
                if error.requires_grad:
                    error.backward()
                return error
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:  # W only
            with torch.enable_grad():
                W = W.detach().requires_grad_()
                theta = theta.detach().requires_grad_()
                fn_at_theta = weightedOrthoError(W, p2d, p3d, theta, optimize_s=False)  # b
...
class NonlinearWeightedOrthBlindPnP(torch.nn.Module):
    def __init__(self):
        super(NonlinearWeightedOrthBlindPnP, self).__init__()

    def forward(self, W, p2d, p3d, theta0=None, optimize_s=True):
        if optimize_s:
            return NonlinearWeightedOrthBlindPnPFn.apply(W, p2d, p3d, theta0)
        else:
            return NonlinearWeightedOrthNosBlindPnPFn.apply(W, p2d, p3d, theta0)

...
def angle_axis_to_rotation_matrix(angle_axis):
    ...
    Rodrigues' Rotation Formula: Implements a common mathematical technique for converting angle-axis
                                 representations into rotation matrices.
    Taylor Series Approximation: A fallback method for small angles where numerical stability is important.
...
def normalise_points(p):
    """This normalizes a batch of points using L2 normalization"""
    return torch.nn.functional.normalize(p, p=2, dim=-1)


def transform_points(p, R, t):
    """This applies a rotation and translation to a batch of points p. It uses the rotation matrix R and
...
def transform_and_normalise_points(p, R, t):
    """This combines the transformation and normalization steps for convenience."""
    return normalise_points(transform_points(p, R, t))


def transform_points_by_theta(p, theta):
    """This takes in a vector θ that contains both rotation (as an angle-axis vector) and translation components,
    transforming the points p accordingly.
    """
...
def project_points_by_theta(p, theta, K=None):
    """This transforms the points using θ and then projects them, potentially using camera intrinsic parameters K."""
...
def points_to_bearings(p, K=None):
    """
    This converts between point representations and bearing vectors, which are used to describe directions in 3D space.
    Arguments:

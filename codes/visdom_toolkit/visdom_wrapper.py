...
class VisPointClouds(VisBase):
    ...
    def draw_data(self):
        ...
        if c is None:
            c = []
            for idx in range(M):
                c.append(torch.zeros((pcds[idx].shape[0],), dtype=torch.int8) + idx)

            c = torch.cat(c).contiguous()+1

        self.visdom.scatter(X=pcds_all, Y=c, opts=dict(title=self.title, markersize=2), win=self.title)

class VisMesh(VisBase):
    def __init__(self, visdom, show_data, title):
        super().__init__(visdom, show_data, title)
    ...
    def draw_data(self):
        pcds = self.raw_data["verts"]
        faces = self.raw_data["faces"]

        if pcds.ndim > 2:
            pcds = pcds[0]

        self.visdom.mesh(X=pcds, Y=faces, opts=dict(title=self.title), win=self.title)
    ...
    def register(self, data, mode, debug_level=0, title='Data', **kwargs):
        if title not in self.registered_blocks.keys():
            ...
            if mode == 'image':
                ...
                self.registered_blocks[title] = VisMesh(self.visdom, show_data, title)
            else:
